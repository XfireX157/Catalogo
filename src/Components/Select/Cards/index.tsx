...
export default function Cards({
  ...
  const filterCard = open ? Modal_Cards.filter((item) => item.id === open) : [];
  ...
  const formattedDiscount = (num: string) =>
    formatNumber(parseInt(num).toFixed(2).replace(".", ","));
        ...
        {itens.map((item) => (
          <Fragment key={item.id}>
            <li className={styles.SelectMenu__options__option}>
              {token !== null && (
                <span className={styles.SelectMenu__options__option__close} onClick={() => deleteCard(item.id!)}>
                  X
                </span>
              )}
              {token !== null && (
              ...
              <article className={styles.SelectMenu__options__option__info}>
                ...
                <div className={styles.SelectMenu__options__option__info__Buy}>
                  <div className={styles.SelectMenu__options__option__info__Buy__price}>
                    <p>{item.category} </p>
                    <div
                      className={styles.SelectMenu__options__option__info__Buy__price__discount}>
